...
export async function GET(request: Request) {
  ...
  const coverDataUris = await Promise.all(topGames.map((game) => fetchCoverAsDataUri(game.coverUrl)));
    ...
    (
      <div
        style={{
          ...
          background: 'linear-gradient(160deg, #17122b 0%, #0d0a1a 55%, #131028 100%)',
          color: '#f3f1fb',
          padding: '110px 80px',
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20 }}>
          ...
          >
            <div style={{ width: 26, height: 26, borderRadius: 13, background: '#8b5cf6' }} />
          </div>
          <div style={{ display: 'flex', fontSize: 34, letterSpacing: 16, textTransform: 'uppercase', color: muted }}>
            SavePoint Wrapped
          </div>
          <div style={{ display: 'flex', fontSize: 108, fontWeight: 700, color: accent }}>{periodLabel}</div>
          <div style={{ display: 'flex', fontSize: 34, color: muted }}>{`@${username}`}</div>
          <div
            style={{
              display: 'flex',
              marginTop: 6,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 22, width: '100%' }}>
          {[stats.slice(0, 2), stats.slice(2, 4)].map((row, rowIndex) => (
            <div key={rowIndex} style={{ display: 'flex', gap: 22, width: '100%' }}>
              {row.map((stat) => (
                <div key={stat.label} style={{ ...tileStyle, flex: 1 }}>
                  <div style={{ display: 'flex', fontSize: 96, fontWeight: 700, color: accent }}>
                    {String(stat.value)}
                  </div>
                  <div style={{ display: 'flex', fontSize: 32, color: muted }}>{stat.label}</div>
                </div>
              ))}
            </div>
          ))}
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14, justifyContent: 'center', marginTop: 8 }}>
              {wrapped.topGenres.slice(0, 5).map((g) => (
                <div
                  ...
                  style={{
                    display: 'flex',
                    background: 'linear-gradient(90deg, rgba(139,92,246,0.28), rgba(245,169,127,0.24))',
                    color: '#e7ddff',
                    borderRadius: 999,
                    padding: '12px 28px',
                    fontSize: 30,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20, width: '100%' }}>
          ...
          {topGames.map((game, index) => (
            ...
            >
              ...
              {coverDataUris[index] ? (
                ...
                <div style={{ display: 'flex', width: 150, height: 90, borderRadius: 16, background: '#241d3d' }} />
              )}
              <div style={{ display: 'flex', flex: 1, overflow: 'hidden', whiteSpace: 'nowrap' }}>
                {game.name}
              </div>
          ...
          {topGames.length === 0 && (
            <div style={{ display: 'flex', justifyContent: 'center', fontSize: 36, color: muted }}>
