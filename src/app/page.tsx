...
import { Star, Trophy, Users, BookMarked, BarChart3, Sparkles } from 'lucide-react';
...
const FEATURES = [
  { icon: Star, title: 'Rate & review', body: 'Score every game 1–5 and write full reviews.' },
  { icon: BookMarked, title: 'Track & log', body: 'A diary of everything you play, auto-updated.' },
  { icon: Users, title: 'Follow friends', body: 'See what your friends are playing right now.' },
  { icon: BarChart3, title: 'Stats & Wrapped', body: 'Genre breakdowns and a shareable yearly recap.' },
];
    ...
    <div className="relative flex min-h-screen flex-1 flex-col overflow-hidden bg-app-glow">
      ...
      <main className="relative mx-auto flex w-full max-w-3xl flex-1 flex-col items-center justify-center gap-7 px-4 py-16 text-center">
        <span className="inline-flex items-center gap-2 rounded-full border border-primary/30 bg-primary/10 px-3 py-1 text-xs font-medium text-primary">
          <Sparkles className="size-3.5" /> Your gaming life, all in one save file
        </span>

        <h1 className="text-balance text-5xl font-bold tracking-tight sm:text-6xl">
          Press <span className="text-gradient-brand">start</span> on your
          <br /> gaming journal.
        </h1>

        <p className="max-w-xl text-balance text-lg text-muted-foreground">
          SavePoint is where players rate, review, and remember every game they touch. Import
          your library, build your backlog, flex your stats, and never lose track of a save
          point again. 🎮
        </p>
      ...
      <footer className="mx-auto w-full max-w-6xl px-4 py-6 text-center text-xs text-muted-foreground">
        <p>
          Developed by <span className="font-medium text-foreground">andimags</span> · SavePoint
          v1.0.0 (stable)
        </p>
      </footer>
    </div>
