...
function DiaryActivityCard({ item }: { item: Extract<ActivityItem, { type: 'diary' }> }) {
  const { diary } = item;
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border bg-card p-3">
      ...
      <div className="min-w-0 flex-1 text-sm">
        <Link href={`/users/${diary.author.id}`} className="font-medium hover:underline">
          {diary.author.username}
        </Link>{' '}
        <span className="text-muted-foreground">
          {diary.status ? `marked as ${STATUS_LABELS[diary.status]}` : 'logged'}
        </span>{' '}
        <Link href={`/games/${diary.game.id}`} className="font-medium hover:underline">
          {diary.game.name}
        </Link>
        {diary.note && <p className="truncate text-xs text-muted-foreground">{diary.note}</p>}
        {loggedAt && <p className="mt-0.5 text-xs text-muted-foreground">{loggedAt}</p>}
      </div>
...
function ListActivityCard({ item }: { item: Extract<ActivityItem, { type: 'list' }> }) {
  const { list } = item;
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border bg-card p-3">
      ...
      <div className="min-w-0 flex-1 text-sm">
        <Link href={`/users/${list.author.id}`} className="font-medium hover:underline">
        ...
        {createdAt && <p className="mt-0.5 text-xs text-muted-foreground">{createdAt}</p>}
      </div>
...
export function HomeClient() {
  ...
  const [friendsFeed, setFriendsFeed] = useState<PlayingFeedItem[] | null>(null);
  ...
  useEffect(() => {
    if (!token) return;
    getPlayingFeed(token).then(setFriendsFeed).catch(() => setFriendsFeed([]));
    getRecommendations(token).then(setRecs).catch(() => setRecs([]));
  }, [token]);
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      setActivity((prev) => {
        ...
        const next = data.items.filter((i) => !seen.has(activityKey(i)));
        return [...prev, ...next];
      });
            ...
            {friendsFeed.map((item, i) => (
              ...
              >
                ...
                <div className="min-w-0">
                  <p className="truncate text-xs font-medium">{item.game.name}</p>
      ...
      <section className="space-y-4">
        ...
        {!initialized ? (
          ...
          <p className="py-8 text-center text-sm text-muted-foreground">
            No activity yet. Log a game or write a review to get things started.
          </p>
        ) : (
          <div className="space-y-4">
            {activity.map((item) => {
              if (item.type === 'review') {
                return <ReviewCard key={`r-${item.review.id}`} review={item.review} showGame />;
              }
              if (item.type === 'diary') {
                return <DiaryActivityCard key={`d-${item.diary.id}`} item={item} />;
              }
              return <ListActivityCard key={`l-${item.list.id}`} item={item} />;
            })}
          </div>
        )}
