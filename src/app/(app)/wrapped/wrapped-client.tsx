...
import { getWrapped, thumbnailUrl, wrappedTagline, type Wrapped } from '@/lib/api-client';
...
export function WrappedClient() {
  ...
  const refresh = useCallback(async () => {
    if (!token) return;
    setWrapped(null);
    setWrapped(await getWrapped(token, year, month === 'all' ? undefined : month));
  }, [token, year, month]);
  ...
  const periodLabel = month === 'all' ? String(year) : `${MONTHS[month - 1]} ${year}`;
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button asChild variant="outline" size="sm" className="ml-auto">
          ...
          >
            <Download data-icon="inline-start" /> Download story image
          </a>
        </Button>
      </div>
          ...
          <CardContent className="relative overflow-hidden bg-app-glow px-7 py-9">
            ...
            <div className="relative space-y-8">
              <div className="flex flex-col items-center gap-2 text-center">
                ...
                <p className="text-4xl font-bold tracking-tight text-gradient-brand">{periodLabel}</p>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { icon: Gamepad2, value: wrapped.totalGames, label: 'games played' },
                  { icon: Clock, value: Math.round(wrapped.totalMinutes / 60), label: 'hours gaming' },
                  { icon: PenLine, value: wrapped.reviewCount, label: 'reviews written' },
                  { icon: Star, value: wrapped.ratingCount, label: 'games rated' },
