...
function UserRow({
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border bg-card p-3">
      ...
      <Link href={`/users/${user.id}`} className="min-w-0 flex-1 truncate font-medium hover:underline">
        {user.username}
      </Link>
      {onToggle && (
        <Button variant={following ? 'outline' : 'default'} size="sm" onClick={onToggle}>
...
export function FriendsClient() {
  ...
  const loadLists = useCallback(async () => {
    ...
    const [fl, fr] = await Promise.all([getFollowing(token, myId), getFollowers(token, myId)]);
    setFollowing(fl);
    setFollowers(fr);
  }, [token, myId]);
  ...
  async function toggleFollow(user: UserSearchResult) {
    ...
    try {
      ...
      setResults((prev) =>
        prev?.map((u) => (u.id === user.id ? { ...u, isFollowing: !u.isFollowing } : u)) ?? null,
      );
      await loadLists();
    } catch (error) {
      ...
      <Tabs defaultValue="following">
        ...
        <TabsContent value="following" className="space-y-2 pt-2">
          {following.length === 0 ? (
            ...
            following.map((u) => <UserRow key={u.id} user={u} following />)
          )}
        </TabsContent>
        <TabsContent value="followers" className="space-y-2 pt-2">
          {followers.length === 0 ? (
            ...
            followers.map((u) => <UserRow key={u.id} user={u} following />)
          )}
        </TabsContent>
      </Tabs>
    </div>
