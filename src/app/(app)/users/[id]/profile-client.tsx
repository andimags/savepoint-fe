  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground">
            {profile.gameCount} games · {profile.followerCount} followers ·{' '}
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {stats.favoriteGame && (
              <Card className="overflow-hidden md:col-span-1">
                <Link href={`/games/${stats.favoriteGame.id}`} className="group block">
                  <div className="relative aspect-[16/9] bg-muted">
                    {stats.favoriteGame.coverUrl && (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img
                        src={thumbnailUrl(stats.favoriteGame.coverUrl) ?? stats.favoriteGame.coverUrl}
                        alt=""
                        loading="lazy"
                    ...
                    <span className="absolute left-2 top-2 inline-flex items-center gap-1 rounded-full bg-brand px-2 py-0.5 text-xs font-medium text-brand-foreground">
                      <Heart className="size-3 fill-current" /> Favorite game
                    </span>
                  </div>
                  <div className="p-3">
                    <p className="truncate text-sm font-medium">{stats.favoriteGame.name}</p>
                  </div>
                </Link>
              </Card>
            )}

            <Card className={stats.favoriteGame ? 'md:col-span-2' : 'md:col-span-3'}>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {stats.favoriteGenres.map((genre) => (
                        <Badge key={genre} className="border-transparent bg-primary/15 text-primary">
                          {genre}
                        </Badge>
                      ))}
                    </div>
                ...
                {stats.favoriteGames.length > 0 && (
                  <div>
                    <p className="mb-2 flex items-center gap-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">
                      <Star className="size-3.5" /> Favorite games
                    </p>
                    <div className="grid grid-cols-5 gap-2">
                      {stats.favoriteGames.map((g) => (
                        <Link key={g.id} href={`/games/${g.id}`} className="group space-y-1">
                          <div className="aspect-[16/9] overflow-hidden rounded-md bg-muted">
                            {g.coverUrl && (
                              // eslint-disable-next-line @next/next/no-img-element
                              <img
                                src={thumbnailUrl(g.coverUrl) ?? g.coverUrl}
                                alt={g.name}
                                loading="lazy"
                                className="size-full object-cover transition-transform duration-300 group-hover:scale-105"
                              />
      ...
      <Tabs defaultValue="games">
        <TabsList>
          <TabsTrigger value="games">Games ({profile.gameCount})</TabsTrigger>
          <TabsTrigger value="lists">Lists ({lists?.length ?? 0})</TabsTrigger>
          <TabsTrigger value="reviews">Reviews ({reviews?.total ?? 0})</TabsTrigger>
          <TabsTrigger value="diary">Diary</TabsTrigger>
          <TabsTrigger value="followers">Followers ({profile.followerCount})</TabsTrigger>
          <TabsTrigger value="following">Following ({profile.followingCount})</TabsTrigger>
        </TabsList>

        <TabsContent value="games" className="pt-4">
          {!games || games.items.length === 0 ? (
            <p className="text-sm text-muted-foreground">No games in library yet.</p>
          ) : (
                ...
                >
                  <div className="aspect-[16/9] w-full overflow-hidden bg-muted">
                    {ug.game.coverUrl ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img
                        src={thumbnailUrl(ug.game.coverUrl) ?? ug.game.coverUrl}
                        alt=""
                        loading="lazy"
                        className="size-full object-cover transition-transform duration-300 group-hover:scale-105"
                      />
                  ...
                  <div className="space-y-1 p-2.5">
                    <p className="truncate text-sm font-medium">{ug.game.name}</p>
                    {ug.status && (
        ...
        <TabsContent value="reviews" className="space-y-4 pt-4">
          {!reviews || reviews.items.length === 0 ? (
            ...
            reviews.items.map((review) => <ReviewCard key={review.id} review={review} showGame />)
          )}
        </TabsContent>
          ...
          {!stats || stats.recentDiary.length === 0 ? (
            ...
            stats.recentDiary.map((entry) => (
              <div key={entry.id} className="flex items-center gap-3 rounded-lg border bg-card p-3">
                <span className="w-16 shrink-0 text-xs text-muted-foreground">{entry.playedOn}</span>
                <Link
        ...
        <TabsContent value="followers" className="space-y-2 pt-4">
          <UserList users={followers} emptyLabel="No followers yet." />
        </TabsContent>
        <TabsContent value="following" className="space-y-2 pt-4">
          <UserList users={following} emptyLabel="Not following anyone yet." />
        </TabsContent>
      </Tabs>
    </div>
  );
...
function UserList({ users, emptyLabel }: { users: SimpleUser[] | null; emptyLabel: string }) {
