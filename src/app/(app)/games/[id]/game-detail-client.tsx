...
export function GameDetailClient({ gameId }: { gameId: string }) {
  ...
  async function toggleFavorite() {
    ...
    try {
      ...
      await updateProfile(token, { favoriteGameId: next ? gameId : null });
      setIsFavorite(next);
      toast.success(next ? 'Set as your favorite game.' : 'Removed from favorite.');
    } catch (error) {
  ...
  async function handleStatusChange(value: string) {
    ...
    try {
      ...
      const updated = await setUserGameStatus(token, entry.id, value as GameStatus);
      setUserGame(updated);
  ...
  async function handleLogDiary(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to log entry.');
    }
  }
      ...
      <div className="flex flex-col gap-6 md:flex-row">
        ...
        <div className="min-w-0 flex-1 space-y-4">
          ...
          <div className="flex items-center gap-3">
            <StarRating value={rating?.userRating ?? null} onChange={handleRate} />
            <span className="text-sm text-muted-foreground">
              {rating?.average != null ? `${rating.average} avg (${rating.count})` : 'No ratings yet'}
            </span>
          </div>

          <div className="flex flex-wrap items-center gap-2">
            <Select value={userGame?.status ?? ''} onValueChange={handleStatusChange}>
            ...
            <Dialog open={diaryOpen} onOpenChange={setDiaryOpen}>
              ...
              <DialogContent>
                ...
                <form onSubmit={handleLogDiary} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={diaryPlatform} onValueChange={setDiaryPlatform}>
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
            ...
            <AddToList gameId={gameId} lists={lists} size="default" variant="outline" />
            ...
            >
              <Heart
                ...
                className={isFavorite ? 'fill-brand text-brand' : ''}
              />
