...
import { getBrowseGames, searchGames, thumbnailUrl, type Game } from '@/lib/api-client';
  ...
  return (
    <div className="space-y-6">
      ...
      {error ? (
        ...
        <p className="text-sm text-muted-foreground">No games found for &ldquo;{query}&rdquo;.</p>
      ) : (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
          {games.map((game) => (
            <div
            ...
            >
              <Link href={`/games/${game.id}`} className="group relative block aspect-[16/9] bg-muted">
                {game.coverUrl ? (
              ...
              <div className="flex flex-1 flex-col gap-2 p-3">
                <Link href={`/games/${game.id}`} className="min-w-0">
                  <p className="truncate text-sm font-medium hover:underline">{game.name}</p>
