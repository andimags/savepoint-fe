...
const PLATFORM_OPTIONS: { value: Platform; label: string }[] = MANUAL_PLATFORMS.map((p) => ({
  value: p,
...
export function LibraryClient() {
  ...
  async function handleRemoveGame() {
    ...
    try {
      ...
      toast.success(`${pendingRemoval.game.name} removed from your library.`);
    ...
    <div className="space-y-4">
      ...
      {!data ? (
        ...
        <p className="py-12 text-center text-sm text-muted-foreground">
          Nothing here yet. Add games manually or connect Steam in Settings → Connections.
        </p>
      ) : (
        <>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {data.items.map((userGame) => (
              ...
              >
                <Link href={`/games/${userGame.gameId}`} className="shrink-0">
                  {userGame.game.coverUrl ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img
                      src={thumbnailUrl(userGame.game.coverUrl) ?? userGame.game.coverUrl}
                      alt=""
                      loading="lazy"
                ...
                <Select
                  ...
                  onValueChange={(value) => handlePlatformChange(userGame, value)}
                >
                ...
                <Select
                  ...
                  onValueChange={(value) => handleStatusChange(userGame, value)}
                >
                  <SelectTrigger size="sm" className="w-32 shrink-0">
      ...
      <Dialog open={pendingRemoval !== null} onOpenChange={(open) => !open && setPendingRemoval(null)}>
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleRemoveGame} disabled={removing}>
