  ...
  if (data.items.length === 0) {
    return (
      <p className="py-12 text-center text-sm text-muted-foreground">
        Your diary is empty. Open a game page and hit “Log to diary” to start tracking what you
        play.
      </p>
    );
  }
          ...
          <div className="space-y-2">
            {entries.map((entry) => (
              <div key={entry.id} className="flex items-center gap-3 rounded-lg border bg-card p-3">
                ...
                <Link href={`/games/${entry.gameId}`} className="shrink-0">
                  {entry.game.coverUrl ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img
                      src={thumbnailUrl(entry.game.coverUrl) ?? entry.game.coverUrl}
                      alt=""
