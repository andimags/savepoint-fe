  ...
  return (
    <div
      className={`flex h-full flex-col items-center gap-3 rounded-2xl border bg-card p-6 text-center transition-all ${
        dimmed ? 'opacity-70' : 'hover:-translate-y-0.5 hover:border-primary/50 hover:shadow-lg hover:shadow-primary/5'
      }`}
    >
      <div
      ...
      <div className="space-y-0.5">
        ...
        <p className="text-xs text-muted-foreground">{subtitle ?? 'Import your library'}</p>
      </div>
...
export function ConnectionsClient() {
  ...
  useEffect(() => {
    if (!status?.connected || !SYNCING_STATES.has(status.syncStatus ?? '')) return;
  ...
  async function handleConnect(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Could not connect Steam account.');
    } finally {
      setConnecting(false);
    }
  }
  ...
  async function handleRefreshData() {
    ...
    toast.success('Refreshing game details (genres, art) in the background.');
  }
  ...
  return (
    <div className="grid grid-cols-2 items-stretch gap-4 sm:grid-cols-3">
      ...
      <Tile
        ...
        glyph={STEAM.glyph}
        name="Steam"
        subtitle={connected ? 'Owned games & playtime' : 'Import your library'}
      >
        {connected ? (
          <div className="flex flex-col items-center gap-2">
            ...
            <div className="flex gap-1.5">
              <Button
                ...
                disabled={SYNCING_STATES.has(status?.syncStatus ?? '')}
              >
                <RefreshCw data-icon="inline-start" /> Resync
              </Button>
              <Button variant="ghost" size="xs" onClick={handleRefreshData}>
                Refresh data
              </Button>
            </div>
          </div>
          ...
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Paste your Steam profile URL, vanity name, or SteamID64. Your profile&apos;s
                  &quot;game details&quot; privacy setting must be public.
                </DialogDescription>
              </DialogHeader>
        ...
        {status?.syncError && (
          <Alert className="mt-1 text-left">
            <AlertDescription className="text-xs">{status.syncError}</AlertDescription>
          </Alert>
        )}
      </Tile>
