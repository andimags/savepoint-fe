...
export type Platform = 'STEAM' | 'GOG' | 'EPIC' | 'XBOX' | 'PLAYSTATION' | 'NINTENDO' | 'OTHER';
...
export interface StatsOverview {
  ...
  statusCounts: { finished: number; playing: number; backlog: number; dropped: number };
...
export interface Wrapped {
  ...
  totalGames: number;
  totalMinutes: number;
  topGames: { id: string; name: string; coverUrl: string | null; playtimeMinutes: number }[];
 ...
 */
export function wrappedTagline(topGenre: string | undefined, totalGames: number): string {
...
export const getGame = (token: string, id: string) => apiFetch<Game>(token, `/games/${id}`);
export const getBrowseGames = (token: string) => apiFetch<Game[]>(token, '/games/browse');
...
export const setUserGameStatus = (token: string, id: string, status: GameStatus | null) =>
  apiFetch<UserGame>(token, `/user-games/${id}`, {
    method: 'PATCH',
    body: JSON.stringify({ status }),
  });
export const setUserGamePlatform = (token: string, id: string, platform: Platform) =>
  apiFetch<UserGame>(token, `/user-games/${id}`, {
    method: 'PATCH',
...
export const setRating = (token: string, gameId: string, value: number) =>
  apiFetch(token, `/games/${gameId}/rating`, { method: 'PUT', body: JSON.stringify({ value }) });
...
export const getGameReviews = (token: string, gameId: string, page = 1) =>
  apiFetch<Paginated<ReviewView>>(token, `/games/${gameId}/reviews?page=${page}`);
export const getRecentReviews = (token: string, page = 1, limit = 10) =>
  apiFetch<Paginated<ReviewView>>(token, `/reviews/recent?page=${page}&limit=${limit}`);
export const getUserReviews = (token: string, userId: string, page = 1, limit = 10) =>
  apiFetch<Paginated<ReviewView>>(token, `/users/${userId}/reviews?page=${page}&limit=${limit}`);
...
export const updateReview = (token: string, reviewId: string, body: string) =>
  apiFetch(token, `/reviews/${reviewId}`, { method: 'PATCH', body: JSON.stringify({ body }) });
...
export const getReviewComments = (token: string, reviewId: string, page = 1) =>
  apiFetch<Paginated<CommentView>>(token, `/reviews/${reviewId}/comments?page=${page}&limit=50`);
export const addReviewComment = (token: string, reviewId: string, body: string) =>
...
// --- Lists ---
export const getMyLists = (token: string) => apiFetch<ListSummary[]>(token, '/lists/mine');
export const getUserLists = (token: string, userId: string) =>
  apiFetch<ListSummary[]>(token, `/lists/user/${userId}`);
export const getList = (token: string, id: string) => apiFetch<ListDetail>(token, `/lists/${id}`);
export const createList = (token: string, title: string, description?: string) =>
  apiFetch<ListSummary>(token, '/lists', {
    method: 'POST',
    body: JSON.stringify({ title, description }),
  });
export const updateList = (token: string, id: string, title: string, description?: string) =>
  apiFetch(token, `/lists/${id}`, { method: 'PATCH', body: JSON.stringify({ title, description }) });
...
export const addListItem = (token: string, listId: string, gameId: string) =>
  apiFetch(token, `/lists/${listId}/items`, { method: 'POST', body: JSON.stringify({ gameId }) });
export const removeListItem = (token: string, listId: string, itemId: string) =>
  apiFetch<void>(token, `/lists/${listId}/items/${itemId}`, { method: 'DELETE' });

// --- Diary ---
export const getDiary = (token: string, page = 1, limit = 50) =>
  apiFetch<Paginated<DiaryEntry>>(token, `/diary?page=${page}&limit=${limit}`);
export const addDiaryEntry = (
  token: string,
  entry: { gameId: string; playedOn: string; platform: string; note?: string },
) => apiFetch<DiaryEntry>(token, '/diary', { method: 'POST', body: JSON.stringify(entry) });
export const updateDiaryEntry = (
  token: string,
  id: string,
  entry: { playedOn?: string; platform?: string; note?: string },
) => apiFetch<DiaryEntry>(token, `/diary/${id}`, { method: 'PATCH', body: JSON.stringify(entry) });
...
) => apiFetch<Me>(token, '/users/me', { method: 'PATCH', body: JSON.stringify(updates) });
...
export const searchUsers = (token: string, q: string) =>
  apiFetch<UserSearchResult[]>(token, `/users/search?q=${encodeURIComponent(q)}`);
...
export const getFollowers = (token: string, userId: string) =>
  apiFetch<{ id: string; username: string }[]>(token, `/users/${userId}/followers`);
export const getFollowing = (token: string, userId: string) =>
  apiFetch<{ id: string; username: string }[]>(token, `/users/${userId}/following`);
...
export const getWrapped = (token: string, year: number, month?: number) =>
  apiFetch<Wrapped>(token, `/stats/wrapped?year=${year}${month ? `&month=${month}` : ''}`);
